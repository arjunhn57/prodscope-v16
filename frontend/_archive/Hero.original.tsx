// ─────────────────────────────────────────────────────────────────────────────
// ARCHIVED: Original Hero.tsx — saved before the bronze-on-bone redesign.
// Kept as reference in case we want to bring back pieces of the prior hero
// (grid pattern, mouse-reveal mask, LineCycler, marquee). NOT imported by
// the app; _archive/ is outside src/ so Vite will not compile this file.
// ─────────────────────────────────────────────────────────────────────────────
...
function IconRow({
  ...
  const renderItems = () =>
    items.map((src, i) => (
      <div
        key={i}
        className="h-16 w-16 flex-shrink-0 rounded-full bg-white shadow-md flex items-center justify-center transition-transform duration-200 hover:scale-[1.15]"
      >
...
/* ── Word Cycler (from animated-hero) ──────────────────────────────────────── */
  ...
  return (
    <span className="inline-grid overflow-hidden">
      ...
      {words.map((word, i) => (
        ...
        >
          {prefix}<span className="font-bold bg-gradient-to-r from-accent via-accent-decorative to-indigo-400 bg-clip-text text-transparent">{word}</span>
        </motion.span>
      ))}
    </span>
  );
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-[1120px] w-full px-6 pt-28 pb-8 md:pt-36 md:pb-12">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="font-heading text-[40px] md:text-[56px] lg:text-[72px] font-bold tracking-tighter leading-[1.05] max-w-3xl mx-auto text-text-primary">
            <motion.span
            ...
            >
              <LineCycler prefix="Get the " words={cyclingWords} />
            </motion.span>
          </h1>
