...
export function LaunchCTA({
  ...
  const disabled = !ready || submitting || uploading;
  const showSpinner = submitting || uploading;
  ...
  return (
    <div className="flex flex-col items-center gap-2.5">
      <motion.button
        ...
        animate={
          reduceMotion || disabled ? { scale: 1 } : { scale: [1, 1.02, 1] }
        }
        transition={
        ...
        style={{
          background: activeColor ? REPORT_GRADIENTS.hero : "#CBD5E1",
      ...
      >
        {showSpinner ? (
          <>
            ...
            {uploading ? "Uploading APK…" : "Starting…"}
          </>
        ) : (
          <>
            Start Analysis
