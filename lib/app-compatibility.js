...
const ANTI_EMULATOR_PACKAGE_PATTERNS = [
  ...
  // Phase A5 additions (2026-04-26): expanded based on common
  // upload categories that hit Play Integrity / SafetyNet / device
  // attestation:
  //   - Rideshare apps (driver + rider identity verification)
  //   - Cash-app / payment apps with biometric attestation
  //   - Crypto wallets / exchanges
  //   - Identity-verification SDKs (Persona, Onfido, Jumio embeds)
  //   - Streaming additions
  /^com\.ubercab(\.|$)/i,
  /^com\.uber\./i,
  /^com\.lyft\./i,
...
// Phase A5 (2026-04-26): controls whether GMS-dependent apps are rejected
// pre-flight. Pre-A4 (default), the AVD lacks Google Play Services, so
// apps that depend on GSF / FCM / Firebase fail silently or hang on first
// boot. Detecting at upload + rejecting saves users from burning a
// credit on a guaranteed broken crawl.
//
// After A4 ships an AVD with GMS, set REJECT_GMS_APPS=false in env and
// the check downgrades to a debug log. Keeping the detection in place
// (just not rejecting) gives us telemetry on which apps depend on GMS,
// in case GMS-related issues resurface.
const REJECT_GMS_APPS =
  String(process.env.REJECT_GMS_APPS || "true").toLowerCase() !== "false";
 ...
 */
/**
 * Phase A5: detect Google Play Services / Firebase dependency. The AVD
 * we run today is bare AOSP (no GMS), so apps that depend on GMS hang
 * on first boot waiting for `com.google.android.gms` services that
 * don't exist. The `usesGms` flag comes from the manifest parser
 * (signals: GMS-related permissions, com.google.* package, Firebase
 * classnames in aapt output).
 *
 * @param {{usesGms?: boolean}} appProfile
 * @returns {{ reason: string }|null}
 */
function detectGmsDependency(appProfile) {
  if (!appProfile || appProfile.usesGms !== true) return null;
  return {
    reason: "App declares Google Play Services / Firebase dependency",
  };
}
...
function assessCompatibility(appProfile) {
  ...
  // Phase A5 (2026-04-26): GMS / Firebase dependency. The AVD currently
  // ships without Google Play Services, so any app that depends on GSF
  // hangs at first boot. Reject pre-flight with a specific reason.
  // After A4 ships a GMS-bearing AVD, set REJECT_GMS_APPS=false in env
  // and this drops to a logged hint instead of a hard rejection.
  const gmsSignal = detectGmsDependency(appProfile);
  if (gmsSignal && REJECT_GMS_APPS) {
    return {
      crawlable: false,
      quality: "uncrawlable",
      reason: `Requires Google Play Services — ${gmsSignal.reason}`,
      recommendation:
        "This app depends on Google Play Services / Firebase, which is not " +
        "available on the current ProdScope test environment. We're rolling " +
        "out a Google-services-enabled environment soon — check back, or " +
        "contact us if your app is a priority.",
    };
  }
