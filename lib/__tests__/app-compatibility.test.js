...
// ── detectGmsDependency (Phase A5) ────────────────────────────────────────

test("detectGmsDependency: usesGms=true is detected", () => {
  const r = detectGmsDependency({ packageName: "com.example.app", usesGms: true });
  assert.ok(r);
  assert.match(r.reason, /Google Play Services|Firebase/i);
});

test("detectGmsDependency: usesGms=false returns null", () => {
  const r = detectGmsDependency({ packageName: "com.example.app", usesGms: false });
  assert.equal(r, null);
});

test("detectGmsDependency: missing usesGms returns null (backward compat)", () => {
  const r = detectGmsDependency({ packageName: "com.example.app" });
  assert.equal(r, null);
});

test("assessCompatibility: GMS-dependent app rejected pre-flight (default config)", () => {
  // Default REJECT_GMS_APPS is true. App with GMS dependency should be
  // rejected with a clear, user-facing reason that names GSF/Firebase.
  const r = assessCompatibility({
    packageName: "com.example.firebase-app",
    launcherActivity: "com.example.app.MainActivity",
    features: [],
    permissions: ["com.google.android.c2dm.permission.RECEIVE"],
    activities: [],
    usesGms: true,
  });
  assert.equal(r.crawlable, false);
  assert.match(r.reason, /Google Play Services|Firebase/i);
  assert.match(r.recommendation, /Google Play Services|Firebase/i);
});

test("assessCompatibility: GMS check does NOT fire when app has hardware that already rejects", () => {
  // Hardware reject takes precedence — user sees the more specific
  // hardware reason instead of GMS.
  const r = assessCompatibility({
    packageName: "com.example.app",
    launcherActivity: "com.example.app.MainActivity",
    features: ["android.hardware.nfc"],
...
// ── Phase A5 anti-emulator deny-list expansion ───────────────────────────

test("assessCompatibility: rideshare apps (Uber) hit anti-emulator", () => {
  const r = assessCompatibility({
    packageName: "com.ubercab",
    launcherActivity: "com.ubercab.MainActivity",
    features: [],
    activities: [],
  });
  assert.equal(r.crawlable, false);
  assert.match(r.reason, /attestation|anti-emulator/i);
});

test("assessCompatibility: Cash App hits anti-emulator", () => {
  const r = assessCompatibility({
    packageName: "com.squareup.cash",
    launcherActivity: "com.squareup.cash.MainActivity",
    features: [],
    activities: [],
  });
  assert.equal(r.crawlable, false);
...
test("assessCompatibility: crypto wallet (MetaMask) hits anti-emulator", () => {
  const r = assessCompatibility({
    packageName: "io.metamask",
    launcherActivity: "io.metamask.MainActivity",
    features: [],
    activities: [],
  });
  assert.equal(r.crawlable, false);
