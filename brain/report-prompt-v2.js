...
function buildReportPromptV2(params) {
  ...
  return [
    ...
    `   EVERY entry MUST include explanation_md (2-3 sentences on why this matters to a VC /`,
    `   PM diligence reader — describe the actual user friction on the cited screens, not`,
    `   generic platitudes) AND recommendation_md (1-2 sentences naming a concrete remediation`,
    `   step specific enough to assign to an engineer).`,
    `4. ux_issues: Only with cited evidence. Skip if findings are thin. SAME explanation_md`,
    `   and recommendation_md requirement as critical_bugs — every finding earns its place`,
    `   on the page by explaining its impact and pointing at a fix.`,
    `5. coverage_summary: structured numbers + explicit "what we couldn't reach" list.`,
    ``,
    `EXAMPLES of acceptable explanation_md / recommendation_md:`,
    `  GOOD explanation: "The home feed surfaces an empty state with no skeleton or progress`,
    `    indicator (screen_3, screen_7) — a first-time user can't distinguish 'app is loading'`,
    `    from 'app is broken'. This is the most common reason for first-session abandonment`,
    `    in social apps."`,
    `  BAD explanation: "This is a UX problem. Users won't like it." (generic, no anchor)`,
    `  GOOD recommendation: "Replace the blank loading container on screen_3 with a 3-card`,
    `    skeleton placeholder using the existing FeedCard component."`,
    `  BAD recommendation: "Improve the loading experience." (not assignable)`,
    ``,
